...
function setup_rhsso() {
	...
	local SSO_REALM_USERNAME=admin
	local SSO_REALM_PASSWORD=12345
	local SSO_JEE_WEB_CLIENT_ID=jee_web
	local SSO_JEE_API_CLIENT_ID=jee_api
  ...
  curl -v -k -X POST $SSO_AUTH_URL/admin/realms \
	...
	local APPLICATION_ADMIN_URI=https://$SSO_JEE_WEB_CLIENT_ID.$OCP_WILDCARD_DOMAIN
	local APPLICATION_REDIRECT_URI=https://$SSO_JEE_WEB_CLIENT_ID.$OCP_WILDCARD_DOMAIN/*

	curl -v -k -X POST https://$SSO_URL/auth/admin/realms/$SSO_REALM/clients \
	-H "Accept: application/json" \
	-H "Content-Type: application/json;charset=UTF-8" \
	-H "Authorization: Bearer $TKN" \
	--data '{"enabled":true,"attributes":{},"clientId":"'$SSO_JEE_WEB_CLIENT_ID'","protocol":"openid-connect","adminUrl":"'$APPLICATION_ADMIN_URI'","baseUrl":"'$APPLICATION_ADMIN_URI'","secret":"bce5816d-98c4-404f-a18d-bcc5cb005c79","publicClient":false,"consentRequired":true,"fullScopeAllowed":true,"redirectUris":["'$APPLICATION_REDIRECT_URI'"],"webOrigins":["+"]}'
